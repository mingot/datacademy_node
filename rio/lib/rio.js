...
function sendAction(cmd, type, msg, options) {
    ...
    binary(conn)
		...
		console.log('as222');
		console.log(CAPABILITIES.PLAIN);
                if (CAPABILITIES.PLAIN) {
		    // here's where problems are currently
			...
			.tap(function (vars) {
                            ...
                            log(hexy.hexy(vars.loginResponse));
	    ...
	    console.log('asdfghjkl');
		    ...
		    // error message retrieval only, for now
                    log("Data packet");
                    log(vars.dataPacket);

		    var hex = hexy.hexy(vars.dataPacket);
		    var str = vars.dataPacket.toString();

		    log(str);

		    // hackishly find error
		    var index1 = str.indexOf('condition') + 9;
		    var index2 = str.indexOf('condition', index1) + 9;
		    var errorString = str.substring(index2);
		    log(errorString);

                    res = parse_SEXP(vars.dataPacket, 0);

                    if (res.length === 0) {
                        log("Response value: " + errorString);
...
function shutdown(options) {
    sendAction("SHUTDTOWN", CMD_TYPE.SHUTDOWN,
...
// a version of sourceAndEval which doesn't append a function invocation at the end.
function sourceAndEvalString(source, options) {
    options = options || {};
    var cmd;
    cmd = source.replace(/(\r\n)/g, "\n");
//    cmd = 'options(show.error.messages = TRUE)\n' + cmd;
    cmd = 'try(' + cmd + ', silent=TRUE)';
    evaluate(cmd, options);
    log('ASLKJhlalkjfhsa');
    console.log('kjyufdsaoyu');
}
exports.sourceAndEvalString = sourceAndEvalString;
